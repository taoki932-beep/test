...
// ── Microphone ────────────────────────────────────────────────────────────────
// Called directly from button click — no awaits before getUserMedia
function initMic() {
  navigator.mediaDevices.getUserMedia({ audio: true, video: false })
    .then(s => {
      localStream = s;
      document.getElementById('micInitWrap').style.display = 'none';
...
async function startCall() {
  ...
  if (!localStream || !localStream.getAudioTracks().some(t => t.readyState === 'live')) {
    toast('ابتدا میکروفون را فعال کنید'); return;
  }
